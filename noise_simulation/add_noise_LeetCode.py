...
DATASET_PATH = '/path/to/deepseek-coder-noisy/Evaluation/LeetCode/data/20240121-Jul.jsonl'
OUTPUT_PATH = '/path/to/deepseek-coder-noisy/Evaluation/LeetCode/data/20240121-Jul-noisy.jsonl'
...
def process_dataset(input_path, output_path, noise_config, batch_size=32):
    ...
    for idx, item in enumerate(data):
        if "prompt_sft" in item:
            prompt = item["prompt_sft"]    
        if "keyboard_typos" in noise_config:
            item["prompt_sft"] = add_keyboard_typos(prompt, intensity=noise_config["keyboard_typos"])
            prompt = item["prompt_sft"]
        if "synonyms" in noise_config:
            item["prompt_sft"] = add_synonym(prompt, intensity=noise_config["synonyms"])
            prompt = item["prompt_sft"]
    ...
    if "paraphrasing" in noise_config:
        ...
        for idx, result in zip(idx_list, paraphrasing_results):
            noisy_data[idx]["prompt_sft"] = result
